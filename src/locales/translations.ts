export type Language = 'tr' | 'en';

export const translations: Record<Language, Record<string, string>> = {
  tr: {
    // Brand & Top Navigation
    'brand.title': 'SİHİRLİ LAMBA',
    'brand.vtt': 'VTT',
    'brand.openLamp': 'Sihirli Lambayı Aç',
    'nav.scene.map': 'Harita',
    'nav.scene.mapFull': '🗺️ Harita Sahnesi',
    'nav.scene.whiteboard': 'Tahta',
    'nav.scene.whiteboardFull': '🎨 Çizim Tahtası',
    'nav.scene.roleplay': 'Görseller',
    'nav.scene.roleplayFull': '🎭 Rol & Görseller',
    'nav.scene.change': 'Sahne Değiştir',
    'nav.sessions': 'Oturumlar',
    'nav.lamp': 'Lamba',
    'nav.lampTooltip': 'AI Sihirli Lamba (Zindan & Hikaye Üretici)',
    'nav.initiative': 'İnisiyatif',
    'nav.initiativeTooltip': 'Savaş İnisiyatif Sırası Takipçisi',
    'nav.dice': 'Zarlar',
    'nav.diceTooltip': 'Zar Masası & Geçmişi',
    'nav.ambience': 'Ambiyans',
    'nav.ambienceTooltip': 'Zindan Ambiyansları ve Ses Efektleri',
    'nav.chat': 'Sohbet',
    'nav.chatTooltip': 'Canlı Parti Sohbeti ve DM Fısıldama',
    'nav.rules': 'Kurallar',
    'nav.rulesTooltip': 'Oyun Kuralları & Not Defteri',
    'nav.wheel': 'Çark',
    'nav.wheelTooltip': 'Şans Çarkı',
    'nav.asset': 'Varlık',
    'nav.assetTooltip': 'Özel Varlık Çizimi & Token Oluşturma',
    'nav.templates': 'Şablonlar',
    'nav.templatesTooltip': 'Zindan Oda Şablonları Çekmecesi',
    'nav.roomLive': 'Oda Canlı',
    'nav.roomConnected': 'Bağlı',
    'nav.invite': 'Davet',
    'nav.inviteTooltip': 'Çok Oyunculu Canlı Oda',
    'nav.streamPopout': 'Yayın',
    'nav.streamPopoutTooltip': 'Discord / Yayın Penceresi Aç',
    'nav.modePlayer': 'Oyuncu',
    'nav.modeDm': 'DM',
    'nav.modeTooltip': 'Mod Değiştir (DM / Oyuncu)',
    'nav.reset': 'Haritayı Sıfırla',
    'nav.resetConfirm': 'Tüm sahneyi varsayılana sıfırlamak istediğine emin misin?',

    // Toolbar (Left)
    'tool.select': 'Token Seç & Taşı',
    'tool.selectSub': 'Karakter ve eşyaları sürükle',
    'tool.fogReveal': 'Savaş Sisi Aç',
    'tool.fogRevealSub': 'Odayı görünür yap (Sisi Kaldır)',
    'tool.fogHide': 'Savaş Sisi Kapat',
    'tool.fogHideSub': 'Odayı karart (Sis Getir)',
    'tool.roomEdit': 'Oda Düzenle & Bağla',
    'tool.roomEditSub': 'Odaları taşı, Shift ile çoklu seç & sağ tıkla bağla',
    'tool.pan': 'Haritayı Kaydır',
    'tool.panSub': 'Görünümü taşı (Pan)',
    'tool.ruler': 'Cetvel / Menzil Ölçer',
    'tool.rulerSub': 'Mesafe ve kare ölç (ft / metre)',
    'tool.draw': 'Haritaya Çiz',
    'tool.drawSub': 'Canlı kalem',
    'tool.laser': 'Lazer İşaretleyici',
    'tool.laserSub': 'Yayında dikkat çek',
    'tool.redo': 'İleri Al (Ctrl + Y)',
    'tool.undo': 'Geri Al (Ctrl + Z)',
    'tool.grid': 'Grid Çizgilerini Aç / Kapat',
    'tool.clearDrawings': 'Tüm Çizimleri Temizle',

    // Layers & Backgrounds
    'layer.title': 'Katman / Kat',
    'layer.floors': 'Bina Katları & Zeminler',
    'layer.bgSettings': 'Zemin Dokusu & Harita Arka Planı',
    'layer.add': 'Yeni Kat Ekle',
    'layer.bgModalTitle': 'Zemin Arka Planı & Dokusu',
    'layer.bgModalSub': 'Bu kata özel zemin dokusu veya battlemap görseli atayın.',
    'layer.presets': 'Hazır Zemin Dokusu Seçin:',
    'layer.uploadBattlemap': 'Veya Özel Harita Görseli Yükleyin (Battlemap JPG/PNG):',
    'layer.chooseImage': 'Harita Resmi Seç',
    'layer.chooseImageSub': 'Kendi çizdiğiniz veya indirdiğiniz haritayı arkaya koyun',
    'layer.opacity': 'Yüklü Görsel Saydamlığı:',
    'layer.removeImage': 'Görseli Kaldır',
    'layer.save': 'Tamam & Kaydet',
    'layer.editName': 'Kat Adını Düzenle',
    'layer.delete': 'Katı Sil',
    'layer.placeholder': 'Kat Adı (Örn: 2. Kat Zindan)',
    'layer.stone': 'Zindan Taşı',
    'layer.forest': 'Çimen & Orman',
    'layer.wood': 'Ahşap Döşeme',
    'layer.parchment': 'Antik Parşömen',
    'layer.cave': 'Mağara & Volkan',
    'layer.water': 'Okyanus / Deniz',
    'layer.space': 'Derin Uzay',

    // Combat & Initiative
    'combat.title': '⚔️ Savaş İnisiyatif Sırası',
    'combat.round': 'Tur',
    'combat.nextTurn': 'Sıradaki Hamle',
    'combat.rollAll': 'Tüm Zarları At',
    'combat.clear': 'Sırayı Temizle',
    'combat.empty': 'Henüz savaşta kimse yok. Haritadaki veya kasadaki tokenlara sağ tıklayıp "Savaş Sırasına Ekle" deyin!',
    'combat.insBadge': 'İns:',
    'combat.score': 'Skor:',
    'combat.hp': 'HP:',
    'combat.addCustom': 'Yeni Savaşçı Ekle',
    'combat.namePlaceholder': 'Savaşçı Adı...',
    'combat.initBonus': 'İnisiyatif Bonusu (+DEX)',
    'combat.pullFromMap': 'Haritadan Çek',
    'combat.endConfirm': 'Savaşı bitirip inisiyatif sırasını sıfırlamak istediğine emin misin?',

    // Dice Roller
    'dice.title': 'Zar Masası (Sürüklenebilir)',
    'dice.who': 'Zar atan ismi...',
    'dice.mod': 'Bonus:',
    'dice.history': 'Son Atılan Zarlar',
    'dice.clearHistory': 'Temizle',
    'dice.noHistory': 'Henüz zar atılmadı.',
    'dice.crit': '🌟 KRİTİK BAŞARI (NAT 20)!',
    'dice.fumble': '💀 KRİTİK HATA (NAT 1)!',
    'dice.rollResult': 'Zar Sonucu',

    // Soundboard
    'sound.title': 'Ambiyans & Ses Efektleri',
    'sound.liveTag': 'Canlı Ses Tablosu',
    'sound.desc': 'Tüm oyuncuların kulaklığında eşzamanlı çalan zindan sesleri ve özel müzikler.',
    'sound.masterVolume': 'Ana Ses Seviyesi',
    'sound.loopTitle': 'Sürekli Zindan Ambiyansı (Loop)',
    'sound.mute': 'Sustur',
    'sound.rain': 'Fırtına & Yağmur',
    'sound.rainSub': 'Kasvetli ve şimşekli',
    'sound.tavern': 'Taverna & Şömine',
    'sound.tavernSub': 'Sıcak ateş çatırtısı',
    'sound.cave': 'Karanlık Mağara',
    'sound.caveSub': 'Derin zindan uğultusu',
    'sound.war': 'Savaş Davulları',
    'sound.warSub': 'Epik savaş ritmi',
    'sound.customTitle': 'Özel Sesler & Müzikler (DM)',
    'sound.addCustom': 'Yeni Ses Ekle',
    'sound.uploadAudio': 'Bilgisayarından MP3 / WAV Yükle',
    'sound.orUrl': '- VEYA SES LİNKİ GİRİN -',
    'sound.soundNamePlaceholder': 'Ses Başlığı (Örn: Boss Savaş Müziği)',
    'sound.saveUrl': 'Linki Kaydet & Ekle',
    'sound.noCustom': 'Henüz özel ses eklenmedi. DM dilediği MP3 veya ses efektini ekleyebilir.',
    'sound.sfxTitle': 'Anlık Zindan Ses Efektleri (SFX)',
    'sound.sfxSword': 'Kılıç & Çarpışma',
    'sound.sfxMagic': 'Büyü Patlaması',
    'sound.sfxTorch': 'Meşale Ateşi',
    'sound.sfxMonster': 'Canavar Kükremesi',
    'sound.sfxVictory': 'Zafer Fanfarı',
    'sound.sfxDice': 'Zar Yuvarlanışı',

    // Chat
    'chat.title': 'Canlı Sohbet & Fısıldama',
    'chat.channel': 'Kanal:',
    'chat.global': '📢 Herkese Açık (Genel)',
    'chat.whisperDm': '🤫 Sadece DM\'e Fısılda',
    'chat.whisperTo': '🤫 {name}\'e Fısılda',
    'chat.placeholder': 'Mesajınızı yazın... (Enter ile gönder)',
    'chat.noMessages': 'Henüz mesaj yok. Arkadaşlarına selam ver veya DM\'e gizlice fısılda!',

    // Multiplayer Modal
    'mp.title': 'Canlı Çok Oyunculu Oda',
    'mp.subtitle': 'Oyuncuları davet edin, isimlerini düzenleyin ve çizim izinlerini yönetin.',
    'mp.yourName': 'İsminiz / Karakter Unvanınız',
    'mp.changeName': 'İsim değiştir...',
    'mp.save': 'Kaydet',
    'mp.liveStatus': 'Odanız Canlı Yayında!',
    'mp.inviteLink': '🔗 Oyuncular İçin Güvenli Davet Linki:',
    'mp.copy': 'Kopyala',
    'mp.copied': 'Kopyalandı!',
    'mp.connectedPlayers': 'Bağlı Oyuncular',
    'mp.noPlayers': 'Henüz odaya katılan oyuncu yok. Yukarıdaki linki arkadaşlarınıza gönderin!',
    'mp.drawOpen': '✏️ Çizim Açık',
    'mp.drawClosed': '🔒 Çizim Kapalı',
    'mp.kick': 'Odadan At',
    'mp.kickConfirm': '"{name}" adlı oyuncuyu odadan atmak istediğinize emin misiniz?',
    'mp.kickedAlert': '🛡️ Zindan Efendisi (DM) tarafından odadan çıkarıldınız.',
    'mp.endRoom': 'Odayı Kapat & Yayını Sonlandır',
    'mp.hostNewRoom': '🚀 Yeni Çok Oyunculu Oda Başlat (DM)',
    'mp.joinCode': 'Veya Bir Odaya Katıl:',
    'mp.join': 'Katıl',

    // Whiteboard
    'wb.toToken': '🖌️ Token Yap',
    'wb.toTokenTooltip': 'Çizim tahtasındaki resmi alıp yeni bir Token / Varlık oluşturun',
    'wb.exportPng': 'PNG Olarak İndir',
    'wb.clear': 'Tahtayı Sıfırla (Temizle)',
    'wb.brush': 'Fırça (Canlı Kalem)',
    'wb.highlighter': 'Fosforlu Kalem',
    'wb.eraser': 'Silgi',
    'wb.fillBucket': 'Boya Kovası (Doldur)',
    'wb.shapes': 'Geometrik Şekiller',
    'wb.textTool': 'Metin Yaz',
    'wb.selectTool': 'Seç & Taşı & Döndür',
    'wb.uploadImage': 'Resim / Harita Yükle',
    'wb.panTool': 'Tahtayı Kaydır',
    'wb.pages': 'Sayfalar / Katmanlar',
    'wb.newPage': 'Yeni Sayfa Ekle',
    'wb.assetsVault': 'Varlık Çantası (Assets)',
    'wb.themeDark': 'Karanlık Tahta',
    'wb.themeGrid': 'Izgaralı Tahta',
    'wb.themeParchment': 'Parşömen',
    'wb.rotateLeft': '90° Sola Döndür',
    'wb.rotateRight': '90° Sağa Döndür',

    // Token Right Click Menu & Inspector
    'tokenMenu.initiative': '⚔️ Savaş Sırasına Ekle',
    'tokenMenu.toWhiteboard': '🎨 Çizim Tahtasına Aktar',
    'tokenMenu.toVault': '🧰 Gizli Kasaya Al (Statlarıyla)',
    'tokenMenu.inspect': 'Tokenı İncele & Düzenle',
    'tokenMenu.duplicate': 'Kopyasını Çoğalt (+1)',
    'tokenMenu.delete': 'Tokenı Haritadan Sil',
    'tokenInspector.title': 'Token İnceleme & Özellikler',
    'tokenInspector.name': 'Karakter / Yaratık Adı',
    'tokenInspector.hp': 'Can Puanı (HP)',
    'tokenInspector.initiativeBonus': 'İnisiyatif Statı (DEX Bonusu)',
    'tokenInspector.statuses': '🎭 Durum Efektleri (Statuses)',
    'tokenInspector.addCustomStatus': 'Özel Efekt Ekle',
    'tokenInspector.notes': 'Gizli DM Notları (Sadece DM Görür)',

    // Backstage Vault Drawer
    'vault.title': 'DM Gizli Kasası & Varlıklar',
    'vault.subtitle': 'Hazırladığınız tüm canavarlar, NPCler ve tuzaklar burada saklanır.',
    'vault.search': 'Varlık ara (ör: Goblin, Sandık)...',
    'vault.empty': 'Kasada henüz kayıtlı varlık yok. "Varlık Çiz" ile oluşturabilir veya haritadaki bir tokenı sağ tıklayıp kasaya alabilirsiniz.',
    'vault.summon': 'Haritaya Çağır',
    'vault.delete': 'Kasadan Sil',
    'vault.all': 'Tümü',
    'vault.monsters': '🧟 Canavarlar',
    'vault.bosses': '👑 Bosslar',
    'vault.items': '📦 Eşyalar & Sandıklar',
    'vault.npcs': '🧝 NPC & Yoldaşlar',

    // Magic Lamp AI Modal
    'lamp.title': '🪔 AI Sihirli Lamba: FRP Zindan & Hikaye Üretici',
    'lamp.tabEncounter': '⚔️ Savaş & Karşılaşma',
    'lamp.tabStory': '📜 Zindan Hikayesi & Oda',
    'lamp.tabNpc': '👤 NPC & Karakter',
    'lamp.tabLoot': '💎 Hazine & Tuzak',
    'lamp.generate': 'Lamba İllüzyonunu Yarat (Üret)',
    'lamp.generating': 'Cin Zindanı Dokuyor...',
    'lamp.promptPlaceholder': 'Nasıl bir sahne veya canavar istiyorsunuz? (Örn: 3. seviye parti için antik mezarlık pususu)',

    // Rulebook & Campaign Notes Modal
    'rules.title': '📖 Kural Kitabı & Kampanya Notları',
    'rules.tabQuick': '⚡ Hızlı FRP Kuralları',
    'rules.tabNotes': '📝 Özel Not Defteri',
    'rules.tabConditions': '🎭 Durumlar & Efektler',
    'rules.addNote': 'Yeni Not Ekle',
    'rules.dmRules': 'Ortak DM Kuralları',
    'rules.personalNotes': 'Kişisel Oyuncu Notları',
    'rules.noteTitlePlaceholder': 'Not Başlığı...',
    'rules.noteContentPlaceholder': 'Kuralları, zar zorluklarını veya kişisel notlarını buraya yaz...',
    'rules.homebrew': 'Ev Kuralları',
    'rules.story': 'Hikaye Notları',
    'rules.quests': 'Görev & Questler',

    // Wheel of Fortune Modal
    'wheel.title': '🎡 Şans Çarkı',
    'wheel.spin': 'ÇARKI ÇEVİR!',
    'wheel.spinning': 'Çark Dönüyor...',
    'wheel.addOption': 'Seçenek Ekle',
    'wheel.presets': 'Hazır Çark Şablonları:',
    'wheel.winnerTitle': '🎉 Kazanan Sonuç:',
    'wheel.searchPlaceholder': 'Çark veya seçenek ara...',
    'wheel.addNewPreset': 'Yeni Çark Taslağı Ekle',
    'wheel.optionsLabel': 'Çark Seçenekleri / Dilimler',

    // Doodle to Asset Modal
    'doodle.title': '🎨 Özel Varlık & Token Çizim Stüdyosu',
    'doodle.tokenName': 'Varlık / Token Adı:',
    'doodle.size': 'Harita Boyutu:',
    'doodle.folder': 'Kasa Klasörü:',
    'doodle.hpLabel': 'Başlangıç Can Puanı (HP):',
    'doodle.initLabel': 'İnisiyatif Statı (DEX):',
    'doodle.saveToVault': 'Sadece Kasaya Kaydet',
    'doodle.saveAndSummon': 'Kaydet & Haritaya Çağır',

    // Room Templates Drawer
    'roomTemplates.title': 'Zindan Oda Şablonları',
    'roomTemplates.dragHint': 'Haritaya sürükleyip bırakın veya tıklayın',
    'roomTemplates.roomThrone': 'Kraliyet Taht Salonu',
    'roomTemplates.roomCrypt': 'Karanlık Mahzen',
    'roomTemplates.roomMagma': 'Lav Akıntılı Mağara',
    'roomTemplates.roomNature': 'Sarmaşıklı Zindan',
    'roomTemplates.roomCorridor': 'Uzun Zindan Koridoru',
    'roomTemplates.roomTreasure': 'Hazine Odası',
    'roomTemplates.descThrone': 'Büyük ve görkemli mermer salon.',
    'roomTemplates.descCrypt': 'İskeletlerin yattığı kasvetli mezar.',
    'roomTemplates.descMagma': 'Ortasından lav geçen tehlikeli alan.',
    'roomTemplates.descNature': 'Kadim köklerin kapladığı gizli geçit.',
    'roomTemplates.descCorridor': 'Odaları bağlayan 2 birimlik koridor.',
    'roomTemplates.descTreasure': 'Tuzaklı küçük kasa odası.',

    // Session Manager Modal
    'session.title': 'Kampanya & Oturum Yöneticisi',
    'session.saveCurrent': 'Mevcut Oturumu Kaydet',
    'session.namePlaceholder': 'Oturum / Bölüm Başlığı...',
    'session.exportJson': 'Yedek İndir (JSON)',
    'session.importJson': 'Yedek Yükle (JSON)',
    'session.load': 'Yükle',
    'session.delete': 'Sil',
    'session.active': 'Aktif',

    // Roleplay Board & Spotlight
    'roleplay.title': '🎭 Rol & Görsel Panosu',
    'roleplay.addHandout': 'Yeni Belge / Resim Paylaş',
    'roleplay.revealAll': 'Tüm Oyunculara Göster',
    'roleplay.hideAll': 'Gizle',
    'roleplay.uploadArt': 'Görsel Yükle (PNG/JPG)',
    'roleplay.handoutTitlePlaceholder': 'Görsel / Belge Başlığı...',
    'roleplay.handoutNotesPlaceholder': 'Oyuncuların göreceği açıklama...',
    'roleplay.empty': 'Henüz paylaşılan görsel yok. DM buradan sahne resimleri, mektuplar veya NPC portreleri gösterebilir.',
    'roleplay.all': 'Tümü',
    'roleplay.locations': 'Mekanlar & Manzaralar',
    'roleplay.npcs': 'Karakter / NPC Portreleri',
    'roleplay.letters': 'Fermanlar & Mektuplar',
    'roleplay.items': 'Eşyalar & İpuçları',
    'roleplay.project': 'Yayına Yansıt',
    'roleplay.stopProject': 'Yansıtmayı Bitir',
    'roleplay.edit': 'Düzenle',
    // Room Context Menu & Inspector
    'roomMenu.connect': 'Odaları Bağla (Koridor Çek)',
    'roomMenu.disconnect': 'Bağlantıyı Ayır / Kes',
    'roomMenu.connectSeq': 'Seçili Odaları Sırayla Bağla',
    'roomMenu.disconnectAll': 'Tüm Bağlantıları Kes',
    'roomMenu.copy': 'Oda(ları) Kopyala (Ctrl+C)',
    'roomMenu.paste': 'Odaları Yapıştır (Ctrl+V)',
    'roomMenu.edit': 'Odayı Düzenle & Yazı Yaz',
    'roomMenu.delete': 'Seçili Oda(ları) Sil',
    'roomMenu.title': 'Oda Başlığı',
    'roomMenu.floorLabel': 'Zeminde Gözükecek Yazı (Floor Label)',
    'roomMenu.floorLabelPlaceholder': 'Örn: ⚠️ Tuzaklı Zemin / Kadim Kütüphane / Boss Alanı',
    'roomMenu.floorLabelPublic': 'Bu zemin yazısını oyuncular da görsün',
    'roomMenu.layerLabel': 'Bulunduğu Kat / Katman',
    'roomMenu.dmNotes': 'DM Hikaye Açıklaması / Gizli Notlar',
    'roomMenu.dmNotesPlaceholder': 'Odaya girildiğinde yapılacak betimleme veya tuzak notları...',
    'roomMenu.roomBgImage': 'Oda Zemin Görseli / Harita Resmi',
    'roomMenu.deleteImage': 'Görseli Sil',
    'roomMenu.uploadImage': 'Resim Yükle',
    'roomMenu.imageUrlPlaceholder': 'Veya Resim URL\'si yapıştır...',
    'roomMenu.spawnTitle': '🎯 Doğuş / Oluşum Noktası',
    'roomMenu.spawnSub': 'Gizli kasadan çağrılan varlıklar bu odanın merkezine doğar.',
    'roomMenu.dimensions': 'Genişlik x Yükseklik',
    'roomMenu.theme': 'Zemin Teması',
    'roomMenu.themeStone': 'Taş Zemin',
    'roomMenu.themeMagma': 'Lav Akıntısı',
    'roomMenu.themeNature': 'Sarmaşık / Doğa',
    'roomMenu.themeGold': 'Altın / Mermer',
    'roomMenu.hideFog': 'Sisi Kapat (Gizle)',
    'roomMenu.revealFog': 'Sisi Aç (Görünür Yap)',
    'tokenMenu.backstage': '🎭 Sahne Arkasına At (Gizle)',
    'tokenMenu.clone': 'Klonla / Çoğalt (+1)',
    'tokenMenu.revealPlayer': 'Oyunculara Göster',

    'spotlight.title': 'CANLI YAYIN SİNEMATİK GÖRÜNÜMÜ',
    'spotlight.close': 'Yansıtmayı Kapat',
  },

  en: {
    // Brand & Top Navigation
    'brand.title': 'MAGIC LAMP',
    'brand.vtt': 'VTT',
    'brand.openLamp': 'Open Magic Lamp',
    'nav.scene.map': 'Map',
    'nav.scene.mapFull': '🗺️ Map Scene',
    'nav.scene.whiteboard': 'Whiteboard',
    'nav.scene.whiteboardFull': '🎨 Collaborative Whiteboard',
    'nav.scene.roleplay': 'Art & Handouts',
    'nav.scene.roleplayFull': '🎭 Roleplay & Handouts',
    'nav.scene.change': 'Switch Scene',
    'nav.sessions': 'Sessions',
    'nav.lamp': 'Genie Lamp',
    'nav.lampTooltip': 'AI Magic Lamp (Dungeon & Story Generator)',
    'nav.initiative': 'Initiative',
    'nav.initiativeTooltip': 'Combat Initiative Tracker',
    'nav.dice': 'Dice',
    'nav.diceTooltip': 'Dice Tray & Roll History',
    'nav.ambience': 'Ambience',
    'nav.ambienceTooltip': 'Dungeon Ambience & Sound Effects',
    'nav.chat': 'Chat',
    'nav.chatTooltip': 'Live Party Chat & DM Whispers',
    'nav.rules': 'Rulebook',
    'nav.rulesTooltip': 'Quick Rules & Campaign Notepad',
    'nav.wheel': 'Wheel',
    'nav.wheelTooltip': 'Wheel of Fortune',
    'nav.asset': 'Asset',
    'nav.assetTooltip': 'Custom Asset Drawing & Token Creator',
    'nav.templates': 'Templates',
    'nav.templatesTooltip': 'Dungeon Room Templates Drawer',
    'nav.roomLive': 'Live Room',
    'nav.roomConnected': 'Connected',
    'nav.invite': 'Invite',
    'nav.inviteTooltip': 'Live Multiplayer Room',
    'nav.streamPopout': 'Stream',
    'nav.streamPopoutTooltip': 'Open Player / Discord Stream Window',
    'nav.modePlayer': 'Player',
    'nav.modeDm': 'DM',
    'nav.modeTooltip': 'Switch Mode (DM / Player)',
    'nav.reset': 'Reset Map',
    'nav.resetConfirm': 'Are you sure you want to reset the entire canvas to default?',

    // Toolbar (Left)
    'tool.select': 'Select & Move Token',
    'tool.selectSub': 'Drag characters and items',
    'tool.fogReveal': 'Reveal Fog of War',
    'tool.fogRevealSub': 'Make room visible (Clear Fog)',
    'tool.fogHide': 'Hide with Fog of War',
    'tool.fogHideSub': 'Darken room (Bring Fog)',
    'tool.roomEdit': 'Edit & Connect Rooms',
    'tool.roomEditSub': 'Move rooms, Shift for multi-select & right click to connect',
    'tool.pan': 'Pan Map',
    'tool.panSub': 'Move view (Pan)',
    'tool.ruler': 'Ruler / Range Finder',
    'tool.rulerSub': 'Measure distance and grid cells (ft / m)',
    'tool.draw': 'Draw on Map',
    'tool.drawSub': 'Live pen',
    'tool.laser': 'Laser Pointer',
    'tool.laserSub': 'Highlight for stream',
    'tool.redo': 'Redo (Ctrl + Y)',
    'tool.undo': 'Undo (Ctrl + Z)',
    'tool.grid': 'Toggle Grid Lines',
    'tool.clearDrawings': 'Clear All Map Drawings',

    // Layers & Backgrounds
    'layer.title': 'Floor / Layer',
    'layer.floors': 'Building Floors & Layers',
    'layer.bgSettings': 'Floor Texture & Map Background',
    'layer.add': 'Add New Floor',
    'layer.bgModalTitle': 'Floor Background & Texture',
    'layer.bgModalSub': 'Assign custom floor textures or battlemap images to this layer.',
    'layer.presets': 'Select Preset Floor Texture:',
    'layer.uploadBattlemap': 'Or Upload Custom Battlemap Image (JPG/PNG):',
    'layer.chooseImage': 'Choose Map Image',
    'layer.chooseImageSub': 'Place custom drawn or downloaded map underneath',
    'layer.opacity': 'Loaded Image Opacity:',
    'layer.removeImage': 'Remove Image',
    'layer.save': 'Done & Save',
    'layer.editName': 'Edit Floor Name',
    'layer.delete': 'Delete Floor',
    'layer.placeholder': 'Floor Name (e.g. 2nd Floor Dungeon)',
    'layer.stone': 'Dungeon Stone',
    'layer.forest': 'Grass & Forest',
    'layer.wood': 'Wood Planks',
    'layer.parchment': 'Ancient Parchment',
    'layer.cave': 'Cave & Volcanic',
    'layer.water': 'Ocean / Sea',
    'layer.space': 'Deep Space',

    // Combat & Initiative
    'combat.title': '⚔️ Combat Initiative Order',
    'combat.round': 'Round',
    'combat.nextTurn': 'Next Turn',
    'combat.rollAll': 'Roll All Dice',
    'combat.clear': 'Clear Order',
    'combat.empty': 'No combatants yet. Right-click tokens on map or vault to "Add to Initiative"!',
    'combat.insBadge': 'Init:',
    'combat.score': 'Score:',
    'combat.hp': 'HP:',
    'combat.addCustom': 'Add Combatant',
    'combat.namePlaceholder': 'Combatant name...',
    'combat.initBonus': 'Initiative Bonus (+DEX)',
    'combat.pullFromMap': 'Pull from Map',
    'combat.endConfirm': 'Are you sure you want to end combat and reset initiative?',

    // Dice Roller
    'dice.title': 'Dice Tray (Draggable)',
    'dice.who': 'Who is Rolling?',
    'dice.mod': 'Bonus:',
    'dice.history': 'Dice Roll Log History',
    'dice.clearHistory': 'Clear History',
    'dice.noHistory': 'No dice rolled yet.',
    'dice.crit': '🌟 CRITICAL SUCCESS (NAT 20)!',
    'dice.fumble': '💀 CRITICAL FAILURE (NAT 1)!',
    'dice.rollResult': 'Roll Result',

    // Soundboard
    'sound.title': 'Ambience & Sound Effects',
    'sound.liveTag': 'Live Sound Table',
    'sound.desc': 'Synchronized dungeon sounds and custom music playing across all players.',
    'sound.masterVolume': 'Master Volume',
    'sound.loopTitle': 'Continuous Dungeon Ambience (Loop)',
    'sound.mute': 'Mute',
    'sound.rain': 'Storm & Rain',
    'sound.rainSub': 'Gloomy with lightning',
    'sound.tavern': 'Tavern & Hearth',
    'sound.tavernSub': 'Cozy crackling fire',
    'sound.cave': 'Dark Cave',
    'sound.caveSub': 'Deep cavern drone',
    'sound.war': 'War Drums',
    'sound.warSub': 'Epic battle rhythm',
    'sound.customTitle': 'Custom Sounds & Music (DM)',
    'sound.addCustom': 'Add New Sound',
    'sound.uploadAudio': 'Upload MP3 / WAV from Device',
    'sound.orUrl': '- OR ENTER AUDIO URL -',
    'sound.soundNamePlaceholder': 'Sound Title (e.g. Boss Battle Theme)',
    'sound.saveUrl': 'Save Link & Add',
    'sound.noCustom': 'No custom sounds yet. DM can upload any MP3 or sound effect.',
    'sound.sfxTitle': 'Instant Dungeon Sound Effects (SFX)',
    'sound.sfxSword': 'Sword & Clash',
    'sound.sfxMagic': 'Magic Burst',
    'sound.sfxTorch': 'Torch Fire',
    'sound.sfxMonster': 'Monster Roar',
    'sound.sfxVictory': 'Victory Fanfare',
    'sound.sfxDice': 'Dice Rolling',

    // Chat
    'chat.title': 'Live Chat & Whispers',
    'chat.channel': 'Channel:',
    'chat.global': '📢 Public (General)',
    'chat.whisperDm': '🤫 Whisper to DM only',
    'chat.whisperTo': '🤫 Whisper to {name}',
    'chat.placeholder': 'Type your message... (Press Enter to send)',
    'chat.noMessages': 'No messages yet. Greet the party or whisper secretly to the DM!',

    // Multiplayer Modal
    'mp.title': 'Live Multiplayer Room',
    'mp.subtitle': 'Invite players, manage character titles, and configure whiteboard permissions.',
    'mp.yourName': 'Your Name / Character Title',
    'mp.changeName': 'Change name...',
    'mp.save': 'Save',
    'mp.liveStatus': 'Your Room is Live!',
    'mp.inviteLink': '🔗 Secure Player Invite Link:',
    'mp.copy': 'Copy',
    'mp.copied': 'Copied!',
    'mp.connectedPlayers': 'Connected Players',
    'mp.noPlayers': 'No players joined yet. Send the invite link above to your party!',
    'mp.drawOpen': '✏️ Drawing Allowed',
    'mp.drawClosed': '🔒 Drawing Locked',
    'mp.kick': 'Kick Player',
    'mp.kickConfirm': 'Are you sure you want to kick "{name}" from the room?',
    'mp.kickedAlert': 'You have been removed from the room by the DM.',
    'mp.endRoom': 'End Room & Stop Stream',
    'mp.hostNewRoom': '🚀 Start New Multiplayer Room (DM)',
    'mp.joinCode': 'Or Join an Existing Room:',
    'mp.join': 'Join',

    // Whiteboard
    'wb.toToken': '🖌️ Make Token',
    'wb.toTokenTooltip': 'Capture whiteboard drawing and create a new Token / Asset',
    'wb.exportPng': 'Download as PNG',
    'wb.clear': 'Clear Whiteboard',
    'wb.brush': 'Brush (Live Pen)',
    'wb.highlighter': 'Highlighter',
    'wb.eraser': 'Eraser',
    'wb.fillBucket': 'Paint Bucket (Fill)',
    'wb.shapes': 'Geometric Shapes',
    'wb.textTool': 'Write Text',
    'wb.selectTool': 'Select, Move & Rotate',
    'wb.uploadImage': 'Upload Image / Map',
    'wb.panTool': 'Pan Whiteboard',
    'wb.pages': 'Pages / Layers',
    'wb.newPage': 'Add New Page',
    'wb.assetsVault': 'Asset Vault',
    'wb.themeDark': 'Dark Board',
    'wb.themeGrid': 'Grid Board',
    'wb.themeParchment': 'Parchment',
    'wb.rotateLeft': 'Rotate 90° Left',
    'wb.rotateRight': 'Rotate 90° Right',

    // Token Right Click Menu & Inspector
    'tokenMenu.initiative': '⚔️ Add to Initiative Order',
    'tokenMenu.toWhiteboard': '🎨 Send to Whiteboard',
    'tokenMenu.toVault': '🧰 Store to Secret Vault (with Stats)',
    'tokenMenu.inspect': 'Inspect & Edit Token',
    'tokenMenu.duplicate': 'Duplicate Token (+1)',
    'tokenMenu.delete': 'Remove Token from Map',
    'tokenInspector.title': 'Token Inspector & Stats',
    'tokenInspector.name': 'Character / Monster Name',
    'tokenInspector.hp': 'Hit Points (HP)',
    'tokenInspector.initiativeBonus': 'Initiative Stat (DEX Modifier)',
    'tokenInspector.statuses': '🎭 Status Conditions',
    'tokenInspector.addCustomStatus': 'Add Custom Status',
    'tokenInspector.notes': 'Secret DM Notes (Only DM Sees)',

    // Backstage Vault Drawer
    'vault.title': 'DM Secret Vault & Assets',
    'vault.subtitle': 'All your prepared monsters, NPCs, and traps are stored here.',
    'vault.search': 'Search assets (e.g. Goblin, Chest)...',
    'vault.empty': 'No assets in vault yet. Create using "Draw Asset" or right-click any token on map to store.',
    'vault.summon': 'Summon to Map',
    'vault.delete': 'Delete from Vault',
    'vault.all': 'All',
    'vault.monsters': '🧟 Monsters',
    'vault.bosses': '👑 Bosses',
    'vault.items': '📦 Items & Chests',
    'vault.npcs': '🧝 NPCs & Allies',

    // Magic Lamp AI Modal
    'lamp.title': '🪔 AI Magic Lamp: TTRPG Dungeon & Story Generator',
    'lamp.tabEncounter': '⚔️ Combat & Encounter',
    'lamp.tabStory': '📜 Dungeon Lore & Room',
    'lamp.tabNpc': '👤 NPC & Personality',
    'lamp.tabLoot': '💎 Treasure & Trap',
    'lamp.generate': 'Create Illusion (Generate)',
    'lamp.generating': 'Genie is weaving the dungeon...',
    'lamp.promptPlaceholder': 'What kind of scene or monster do you want? (e.g. Level 3 party ambush in an ancient crypt)',

    // Rulebook & Campaign Notes Modal
    'rules.title': '📖 Rulebook & Campaign Notes',
    'rules.tabQuick': '⚡ Quick TTRPG Rules',
    'rules.tabNotes': '📝 Custom Campaign Notepad',
    'rules.tabConditions': '🎭 Conditions & Effects',
    'rules.addNote': 'Add New Note',
    'rules.dmRules': 'Shared DM Rules',
    'rules.personalNotes': 'Personal Player Notes',
    'rules.noteTitlePlaceholder': 'Note Title...',
    'rules.noteContentPlaceholder': 'Notes, game mechanics, or story secrets...',
    'rules.homebrew': 'Homebrew Rules',
    'rules.story': 'Story Notes',
    'rules.quests': 'Quests & Objectives',

    // Wheel of Fortune Modal
    'wheel.title': '🎡 Wheel of Fortune',
    'wheel.spin': 'SPIN THE WHEEL!',
    'wheel.spinning': 'Wheel Spinning...',
    'wheel.addOption': 'Add Option',
    'wheel.presets': 'Preset Wheel Templates:',
    'wheel.winnerTitle': '🎉 Winning Result:',
    'wheel.searchPlaceholder': 'Search wheels or options...',
    'wheel.addNewPreset': 'Add New Wheel Preset',
    'wheel.optionsLabel': 'Wheel Options / Slices',

    // Doodle to Asset Modal
    'doodle.title': '🎨 Custom Asset & Token Art Studio',
    'doodle.tokenName': 'Asset / Token Name:',
    'doodle.size': 'Map Grid Size:',
    'doodle.folder': 'Vault Category:',
    'doodle.hpLabel': 'Starting Hit Points (HP):',
    'doodle.initLabel': 'Initiative Stat (DEX):',
    'doodle.saveToVault': 'Save to Vault Only',
    'doodle.saveAndSummon': 'Save & Summon to Map',

    // Room Templates Drawer
    'roomTemplates.title': 'Dungeon Room Templates',
    'roomTemplates.dragHint': 'Drag & drop onto map or click to add',
    'roomTemplates.roomThrone': 'Royal Throne Hall',
    'roomTemplates.roomCrypt': 'Dark Crypt',
    'roomTemplates.roomMagma': 'Magma Chamber',
    'roomTemplates.roomNature': 'Overgrown Dungeon',
    'roomTemplates.roomCorridor': 'Long Dungeon Corridor',
    'roomTemplates.roomTreasure': 'Treasure Vault',
    'roomTemplates.descThrone': 'Grand and majestic marble hall.',
    'roomTemplates.descCrypt': 'Gloomy crypt where skeletons rest.',
    'roomTemplates.descMagma': 'Hazardous cavern with flowing lava.',
    'roomTemplates.descNature': 'Ancient passage overgrown with roots.',
    'roomTemplates.descCorridor': '2-unit connecting corridor.',
    'roomTemplates.descTreasure': 'Small trapped treasure vault.',

    // Session Manager Modal
    'session.title': 'Campaign & Session Manager',
    'session.saveCurrent': 'Save Current Session',
    'session.namePlaceholder': 'Session / Chapter Title...',
    'session.exportJson': 'Download Backup (JSON)',
    'session.importJson': 'Load Backup (JSON)',
    'session.load': 'Load',
    'session.delete': 'Delete',
    'session.active': 'Active',

    // Roleplay Board & Spotlight
    'roleplay.title': '🎭 Roleplay & Handouts Board',
    'roleplay.addHandout': 'Share New Handout / Art',
    'roleplay.revealAll': 'Reveal to All Players',
    'roleplay.hideAll': 'Hide',
    'roleplay.uploadArt': 'Upload Artwork (PNG/JPG)',
    'roleplay.handoutTitlePlaceholder': 'Artwork / Handout Title...',
    'roleplay.handoutNotesPlaceholder': 'Player-visible description or lore notes...',
    'roleplay.empty': 'No handouts shared yet. DM can reveal scene artwork, letters, or NPC portraits here.',
    'roleplay.all': 'All',
    'roleplay.locations': 'Locations & Scenery',
    'roleplay.npcs': 'Character & NPC Portraits',
    'roleplay.letters': 'Handouts & Letters',
    'roleplay.items': 'Items & Clues',
    'roleplay.project': 'Project to Stream',
    'roleplay.stopProject': 'Stop Projecting',
    'roleplay.edit': 'Edit',
    // Room Context Menu & Inspector
    'roomMenu.connect': 'Connect Rooms (Add Corridor)',
    'roomMenu.disconnect': 'Disconnect Corridor',
    'roomMenu.connectSeq': 'Connect Selected Rooms in Sequence',
    'roomMenu.disconnectAll': 'Disconnect All Corridors',
    'roomMenu.copy': 'Copy Room(s) (Ctrl+C)',
    'roomMenu.paste': 'Paste Room(s) (Ctrl+V)',
    'roomMenu.edit': 'Edit Room & Labels',
    'roomMenu.delete': 'Delete Room(s)',
    'roomMenu.title': 'Room Title',
    'roomMenu.floorLabel': 'Floor Text Label',
    'roomMenu.floorLabelPlaceholder': 'e.g. ⚠️ Trapped Floor / Ancient Library / Boss Lair',
    'roomMenu.floorLabelPublic': 'Show this floor label to players as well',
    'roomMenu.layerLabel': 'Assigned Floor / Level',
    'roomMenu.dmNotes': 'DM Story Description / Secret Notes',
    'roomMenu.dmNotesPlaceholder': 'Sensory descriptions or trap mechanics...',
    'roomMenu.roomBgImage': 'Room Floor Texture / Battlemap Image',
    'roomMenu.deleteImage': 'Remove Image',
    'roomMenu.uploadImage': 'Upload Image',
    'roomMenu.imageUrlPlaceholder': 'Or paste image URL...',
    'roomMenu.spawnTitle': '🎯 Spawn Point',
    'roomMenu.spawnSub': 'Assets summoned from vault will spawn at the center of this room.',
    'roomMenu.dimensions': 'Width x Height (Grid Cells)',
    'roomMenu.theme': 'Floor Theme',
    'roomMenu.themeStone': 'Dungeon Stone',
    'roomMenu.themeMagma': 'Flowing Magma',
    'roomMenu.themeNature': 'Vines & Overgrown',
    'roomMenu.themeGold': 'Marble & Gold',
    'roomMenu.hideFog': 'Hide with Fog',
    'roomMenu.revealFog': 'Reveal Fog (Visible)',
    'tokenMenu.backstage': '🎭 Send to Backstage (Hide)',
    'tokenMenu.clone': 'Clone / Duplicate (+1)',
    'tokenMenu.revealPlayer': 'Reveal to Players',

    'spotlight.title': 'LIVE STREAM CINEMATIC VIEW',
    'spotlight.close': 'Close Spotlight',
  }
};
